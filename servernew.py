...
def realadd():
	...
	for elem in cursor:
		pivot.append(elem)
	cursor.close() # NO SQL INJECTION
	if not pivot and age>0 and (gender=='F' or gender=='f' or gender=='M' or gender=='m'):
...
def useractualchange():
	...
	print "A judge to test if newusername already exists in table", usernamejudge
	#for elem in username_existing:
		#if elem==usernamejudge:
		#	print "True"
		#print "False"
	if usernamejudge not in username_existing and len(newusername)>0 and len(newpsd)>0 and len(newgender)>0 and len(newemailaddress)>0 and len(newage)>0 and (newgender=='f' or newgender=='F' or newgender=='M' or newgender=='m') and int(newage)>0:
		print "ready to change profile"
		cursor=g.conn.execute("update ltuser set username=%s, password=%s, gender=%s, emailaddress=%s, age=%s where userid=%s",newusername,newpsd,newgender.upper(),newemailaddress,newage,uid)
		cursor.close()
...
def commentcheck():
	...
	print "!!!!"
	print newrate
	if newrate.isdigit() and (int(newrate)==1 or int(newrate)==2 or int(newrate)==3 or int(newrate)==4 or int(newrate)==5 or int(newrate)==6 or int(newrate)==7 or int(newrate)==8 or int(newrate)==9 or int(newrate)==10):
		...
		cursor=g.conn.execute("insert into comment_written_about values(%s,%s,%s,current_timestamp,%s,%s)",uid,gameid,int(newid),newcontent,newrate)
		cursor.close()
		user=[]
...
def update_comment_check():
	...
	if newrate.isdigit() and (int(newrate)==1 or int(newrate)==2 or int(newrate)==3 or int(newrate)==4 or int(newrate)==5 or int(newrate)==6 or int(newrate)==7 or int(newrate)==8 or int(newrate)==9 or int(newrate)==10):
		...
		cursor=g.conn.execute("update comment_written_about set content=%s, rate=%s where commentid=%s",newcontent,newrate,commentid)
		cursor.close()
		user=[]
		cursor=g.conn.execute("select username from ltuser where userid=%s",uid)
		for elem in cursor:
